...
def show() -> None:
    ...
    page_header(
        "Team Workspace",
        "Assign ownership, onboard operators, and keep the workspace aligned with enterprise operating roles. "
        "Roles are intentionally business-oriented so platform, FinOps, sustainability, and admin users can "
        "collaborate without sharing the same permissions.",
        icon="\U0001f465",
    )

    if st.button("\U0001f331 Add Demo Team Pack", use_container_width=False, disabled=not can_admin):
    ...
    metric_row(
        [
            ("Members", str(len(members)), "Total workspace roster."),
            ("Plan", getattr(organization, "plan", "starter").title() if organization else "Starter", "Current subscription tier."),
            ("Owners / Admins", str(sum(1 for member in members if member["role"] in {"owner", "admin"})), "Can manage the workspace."),
        ]
    )

    with card("Workspace Team", icon="\U0001f465"):
        ...
        if members:
            ...
            empty_state("No members found in this workspace.", icon="\U0001f465")

    with card("Invite Member", icon="✉️"):
