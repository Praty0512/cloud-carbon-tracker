...
def show() -> None:
    ...
    page_header(
        "Portfolio Workspace",
        "Track project ownership, budget exposure, and carbon performance across your operating estate. "
        "Use it to understand which projects drive emissions, where budgets are concentrated, and which "
        "operating regions deserve attention.",
        icon="\U0001f4bc",
    )
    ...
    metric_row(
        [
            ("Tracked Budget", f"${headline['monthly_budget']:,.0f}", "Registered monthly budget across projects."),
            ("Estimated Spend", f"${headline['estimated_monthly_spend']:,.0f}", "A simple workspace spend proxy."),
            ("Portfolio Carbon", f"{headline['total_carbon']:,.0f} kg CO2", "Total tracked emissions."),
            ("Projects At Risk", str(headline["risk_count"]), "Contributing disproportionately high carbon."),
        ]
    )
    ...
    if portfolio:
        ...
        with card("Project Register", icon="\U0001f4cb"):
            st.caption(
        ...
        empty_state("No portfolio telemetry yet. Seed the workspace or import a dataset to populate this view.", icon="\U0001f4bc")

    with card("Add Project", icon="➕"):
